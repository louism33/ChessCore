...
public class PieceMoveSliding {
    ...
    private static long singleBishopAllMoves(Chessboard board, long piece, boolean white, long legalPushes, long legalCaptures){
        long ALL_PIECES = board.whitePieces() | board.blackPieces(),
                NORTH_WEST = BitBoardUtils.FILE_A | BitBoardUtils.RANK_EIGHT,
                NORTH_EAST = BitBoardUtils.FILE_H | BitBoardUtils.RANK_EIGHT,
                SOUTH_WEST = BitBoardUtils.FILE_A | BitBoardUtils.RANK_ONE,
                SOUTH_EAST = BitBoardUtils.FILE_H | BitBoardUtils.RANK_ONE;
    ...
    private static long singleRookAllMoves(Chessboard board, long piece, boolean white, long legalPushes, long legalCaptures){
        ...
        long temp = piece;
        while (true) {
            if ((temp & BitBoardUtils.FILE_A) != 0) break;
            temp <<= 1;
            answer |= temp;
        ...
        temp = piece;
        while (true) {
            if ((temp & BitBoardUtils.FILE_H) != 0) break;
            temp >>>= 1;
            answer |= temp;
            if ((temp & allPieces) != 0) break;
        }
