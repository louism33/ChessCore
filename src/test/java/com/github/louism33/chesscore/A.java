...
//        String ms = "g1f3, d7d5, d2d4, b8c6, c1f4, g7g5, f4g5, g8f6, g5f6, e7f6, e2e3, f8d6, c2c4, d5c4, f1c4, h8g8, e1f1, c8e6, d4d5, c6e5, d5e6, e5c4, e6f7";
        ...
        for (int i = 0; i < length; i++) {
            int move = buildMoveFromLAN(board, moves[i]);
        ...
        for (int i = 0; i < moves.length; i++) {
            String prettyMove = MoveParser.prettyMove(buildMoveFromLAN(board2, moves[i]));
            int moveAfter = buildMoveFromLAN(board2, prettyMove);
            String myPrettyMove = MoveParser.toString(moveAfter);
            String prettyMove2 = MoveParser.prettyMove(buildMoveFromLAN(board2, myPrettyMove));
            int moveAfter2 = buildMoveFromLAN(board2, prettyMove2);
            
            board2.makeMoveAndFlipTurnBetter(moveAfter2);
        }
