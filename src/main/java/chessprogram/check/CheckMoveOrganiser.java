...
import chessprogram.moveGeneration.*;

import java.util.ArrayList;
import java.util.List;

import static chessprogram.bitboards.BitBoardUtils.*;
import static chessprogram.chessboard.BitIndexing.UNIVERSE;
import static chessprogram.chessboard.BitIndexing.getIndexOfFirstPiece;
...
public class CheckMoveOrganiser {
    ...
    private static List<Move> allLegalCheckEscapeMoves(Chessboard board, boolean white, long ignoreThesePieces) {
        ...
        long PENULTIMATE_RANK = white ? BitBoardUtils.RANK_SEVEN : BitBoardUtils.RANK_TWO;
    ...
    private static long extractRayFromTwoPieces(Chessboard board, long pieceOne, long pieceTwo){
        ...
        while (true) {
            if ((smallPiece & BitBoardUtils.FILE_A) != 0) {
                break;
            }
            smallPiece <<= 1;
        ...
        while (true) {
            if ((smallPiece & BitBoardUtils.RANK_EIGHT) != 0) {
                break;
            }
            smallPiece <<= 8;
            if ((smallPiece & ALL_PIECES_TO_AVOID) != 0) {
    ...
    public static long extractInfiniteRayFromTwoPieces(Chessboard board, long pieceOne, long pieceTwo){
        ...
        thisOne = false;
        while (true) {
            if ((smallPiece & BitBoardUtils.FILE_H) != 0) {
                if (thisOne) {
                    answer |= possibleAnswer;
                }
                break;
        ...
        thisOne = false;
        while (true) {
            if ((smallPiece & BitBoardUtils.RANK_ONE) != 0) {
                if (thisOne) {
                    answer |= possibleAnswer;
                }
                break;
